...
## Prasyarat
- Laragon sudah terpasang, Apache dan MySQL/MariaDB dapat dijalankan, serta database dapat diakses melalui HeidiSQL atau phpMyAdmin.
...
### Primary Key
PRIMARY KEY adalah kolom atau kombinasi kolom yang secara unik mengidentifikasi setiap baris dalam tabel. Hanya satu PRIMARY KEY yang dapat dibuat per tabel. Nilai pada PRIMARY KEY harus unik dan tidak boleh NULL. MySQL/MariaDB secara otomatis membuat indeks pada kolom PRIMARY KEY untuk mendukung pencarian dan integritas data. 
...
> **Catatan Laragon (MySQL/MariaDB):** `NO ACTION`, `CASCADE`, dan `SET NULL` dapat dipraktikkan. `SET DEFAULT` tetap dibahas sebagai konsep referential action, tetapi tidak didukung langsung oleh MySQL/MariaDB.
...
### Unique Key
UNIQUE KEY memastikan semua nilai di kolom tertentu harus unik, tetapi berbeda dengan PRIMARY KEY karena kolom UNIQUE dapat berisi NULL. Suatu tabel bisa memiliki beberapa UNIQUE constraints. MySQL/MariaDB membuat indeks unik secara otomatis untuk kolom dengan constraint UNIQUE. 
...
MySQL/MariaDB yang dijalankan melalui Laragon memiliki beberapa tipe data yang bisa digunakan untuk menyimpan berbagai jenis data sesuai kebutuhan aplikasi. Berikut ini adalah kategori utama tipe data dalam MySQL/MariaDB beserta contoh masing-masing:
...
DELETE FROM target_table LIMIT 10;
